...
func RegisterTransactionRoutes(r *mux.Router, db *gorm.DB) {
	...
	// r.Handle("/api/transaction/tenant/approve/{transaction-id}/", middleware.JwtAuthUser(handlers.(db))).Methods("PATCH")
	// r.Handle("/api/transaction/tenant/get/{transaction-id}", middleware.JwtAuthUser(handlers.(db))).Methods("PATCH")
}
